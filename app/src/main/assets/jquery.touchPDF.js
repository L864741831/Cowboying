			...
			if (!options.disableSwipe) {
				$viewer.swipe( {
					swipe:function(event, direction, distance, duration, fingerCount, fingerData) {
						...
						if (direction == "down") goto(pageNum-1);
						else if (direction == "up") goto(pageNum+1);
					},
					threshold:50,
		...
		*/
		function renderPage () {
			...
			pdfDoc.getPage(pageNumRendering).then(function(page) {
				var viewport = page.getViewport(options.pdfScale*options.quality*0.83);
				canvas.height = viewport.height;
				canvas.width = viewport.width;
				$(".pdf-canvas").css({width: '100%',height: '100%'});
