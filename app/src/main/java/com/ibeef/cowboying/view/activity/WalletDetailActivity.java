 ...
 * 钱包明细
 */
public class WalletDetailActivity extends BaseActivity  implements SuperSwipeRefreshLayout.OnPullRefreshListener,BaseQuickAdapter.RequestLoadMoreListener,IncomeInfoBase.IView {
    @Bind(R.id.back_id)
    ImageView backId;
    @Bind(R.id.info)
    TextView info;
    ...
    private void init(){
        ...
        incomeInfoPresenter.getWalletRecord(reqData,page,"1");
    }
    ...
    public void getWalletRecord(WalletRecordResultBean walletRecordResultBean) {
        if ("000000".equals(walletRecordResultBean.getCode())) {
            if (SDCardUtil.isNullOrEmpty(walletRecordResultBean.getBizData())) {
                ...
                walletDetailListAdapter.loadMoreEnd();
            } else {
                isFirst = false;
                beanList.addAll(walletRecordResultBean.getBizData());
                walletDetailListAdapter.setNewData(this.beanList);
                walletDetailListAdapter.loadMoreComplete();
            }
        } else {
