 ...
 **/
public class PayPwdModel implements PayPwdBase.IModel {
    ...
    @Override
    public Subscription isPayPwd(Map<String, String> headers,final ResponseCallback<PayPwdResultBean> callback) {
        Observable<PayPwdResultBean> observable = service.isPayPwd(headers);

        Subscription sub = observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .retryWhen(new RetryWithDelay(2, 3000))
                //总共重试3次，重试间隔3秒
                .subscribe(new Action1<PayPwdResultBean>() {
                    @Override
                    public void call(PayPwdResultBean result) {
