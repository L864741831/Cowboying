...
public class AssetDetailListAdapter extends BaseQuickAdapter<WalletRecordResultBean.BizDataBean,BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, WalletRecordResultBean.BizDataBean item) {
        ...
        if("11".equals(item.getChangeType())){
            helper.setText(R.id.show_reson_id,"卖牛本金收益返还");
        ...
        }else  if("16".equals(item.getChangeType())||"36".equals(item.getChangeType())){
            helper.setText(R.id.show_reson_id,"订单支付");
        }else  if("17".equals(item.getChangeType())||"37".equals(item.getChangeType())){
            ...
            helper.setText(R.id.show_reson_id,"用户提现减少");
        }else  if("35".equals(item.getChangeType())){
            helper.setText(R.id.show_reson_id,"用户提现失败或拒绝");
        }else  if("38".equals(item.getChangeType())||"18".equals(item.getChangeType())){
            helper.setText(R.id.show_reson_id,"线下门店支付");
        }
